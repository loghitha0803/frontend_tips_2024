...
document.querySelector('.image-city').addEventListener('change', (city) => {
  ...
  if (
    ...
    imageValue.src = '../../../Assets/Icons for cities/anadyr.svg'
    imageVal.forEach(function (image) {
      image.src = '../../../Assets/Icons for cities/anadyr.svg'
    })
    ...
    temperature.innerText = `${cityData[anadyr].temperature}`
    humidityValue.innerText = `${cityData[anadyr].humidity}`
    precipitationValue.innerText = `${cityData[anadyr].precipitation}`
    const [tempvalue] = cityData[anadyr].temperature.split('\u00B0C')
    ...
    function formatDateTime (datetime) {
      ...
      const updatedTimings = cityData[anadyr].nextFiveHrs
      updatedTimings.unshift(cityData[anadyr].temperature)
      ...
      for (let i = 0; i < tempHourlyUpdate.length; i++) { tempHourly[i].textContent = updatedTimings[i] }
    ...
    formatDateTime(cityData[anadyr].dateAndTime)
  } else {
    ...
    const [tempvalue] =
      cityData[givenInput].temperature.split('\u00B0C')
    const farenheitValue = document.querySelector('.temp-value-farenheit')
    const farenheit = Math.round(Number(tempvalue) * 1.8 + 32, 0)
    farenheitValue.innerText = `${farenheit}F`
